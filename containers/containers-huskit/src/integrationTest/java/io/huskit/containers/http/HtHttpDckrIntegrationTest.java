...
class HtHttpDckrIntegrationTest implements DockerIntegrationTest {
    ...
    @Test
    void qwe(HtHttpDckr subject) {
        var container = subject.containers()
                .run(
                        DockerImagesStash.defaultSmall(),
                        spec -> spec.withCommand(DockerImagesStash.smallImageBusyCommand().command(), DockerImagesStash.smallImageBusyCommand().args())
                )
                .exec();
        try {
            ...
            subject.containers().remove(container.id(), spec -> spec.withForce().withVolumes()).exec();
        }
    }
